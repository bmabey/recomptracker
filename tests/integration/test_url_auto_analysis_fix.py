...
def test_url_auto_analysis_fix():
    ...
    with (
        ...
        if not needs_analysis:
            print(
                "❌ FAILURE: url_loaded_needs_analysis flag not set after URL loading"
            )
            return False
        ...
        if mock_session_state.get("url_loaded_needs_analysis", False):
            ...
            if (
                mock_session_state.user_info.get("birth_date")
                and mock_session_state.user_info.get("gender")
                and len(mock_session_state.scan_history) > 0
                and any(scan.get("date") for scan in mock_session_state.scan_history)
            ):
...
def test_url_auto_analysis_flag_cleared():
    ...
    with (
        ...
        if not flag_after and analysis_called["count"] == 1:
            ...
            print(
                f"❌ Flag clearing failed. Flag after: {flag_after}, Analysis calls: {analysis_called['count']}"
            )
            return False
