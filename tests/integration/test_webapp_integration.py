...
class TestWebAppIntegration:
    ...
    def test_webapp_loads_successfully(self, app):
        ...
        assert len(at.exception) == 0, (
            f"No exceptions should occur on load, found: {[e.value for e in at.exception]}"
        )
    ...
    def test_load_example_config_button(self, app, example_config_data):
        ...
        birth_date_str = (
            birth_date_input.value.strftime("%m/%d/%Y")
    ...
    def test_analysis_execution_with_example_config(self, app):
        ...
        assert len(at.exception) == 0, (
            f"Analysis should complete without exceptions, found: {[e.value for e in at.exception]}"
        )

        # Verify no error messages
        error_messages = [error.value for error in at.error]
        assert len(error_messages) == 0, (
            f"No error messages should appear, found: {error_messages}"
        )
    ...
    def test_analysis_results_display(self, app):
        ...
        assert len(metrics) >= 4, (
            "Should display at least 4 metrics (ALMI, ALMI %, FFMI, FFMI %)"
        )
        ...
        for metric in metrics:
            if "ALMI" in metric.label and "kg/m²" in metric.value:
                ...
                assert 5.0 <= almi_value <= 15.0, (
                    f"ALMI value {almi_value} should be reasonable"
                )
            ...
            if "Percentile" in metric.label and "%" in metric.value:
                ...
                assert 0.0 <= percentile <= 100.0, (
                    f"Percentile {percentile} should be valid"
                )
    ...
    def test_form_validation(self, app):
        ...
        if analysis_button:
            ...
            assert len(at.exception) == 0, (
                "Validation should handle empty forms gracefully"
            )
    ...
    def test_csv_export_availability(self, app):
        ...
        assert len(at.exception) == 0, (
            "Analysis completion should enable export functionality"
        )
    ...
    def test_fake_data_generation(self, app):
        ...
        if random_button:
            ...
            assert birth_date_input.value != "", (
                "Birth date should be populated with fake data"
            )

            # Verify no exceptions occurred during fake data generation
            assert len(at.exception) == 0, (
                "Fake data generation should not cause exceptions"
            )
    ...
    def test_plotly_goal_hover_formatting(self):
        ...
        try:
            ...
            df_results, goal_calculations, figures, comparison_table_html = (
                run_analysis_from_data(user_info, scan_history, almi_goal, ffmi_goal)
            )
            ...
            if "almi" in goal_calculations:
                ...
                fig = create_plotly_metric_plot(
                    df_results, "ALMI", lms_functions, goal_calculations
                )
                ...
                assert goal_trace is not None, (
                    "Goal trace should be present in plotly figure"
                )
                assert hasattr(goal_trace, "text"), (
                    "Goal trace should have text attribute for hover"
                )
                # Plotly converts lists to tuples internally, so check for either
                assert isinstance(goal_trace.text, (list, tuple)), (
                    "Goal trace text should be a list or tuple for plotly compatibility"
                )
                assert len(goal_trace.text) == 1, (
                    "Goal trace should have exactly one text item for single data point"
                )
                ...
                assert "Target Body Composition:" in hover_text, (
                    "Hover should contain target body composition section"
                )
                assert "Changes Needed:" in hover_text, (
                    "Hover should contain changes needed section"
                )
                assert "Weight:" in hover_text, (
                    "Hover should contain weight information"
                )
                assert "Lean Mass:" in hover_text, (
                    "Hover should contain lean mass information"
                )

                print(
                    f"✅ Goal hover test passed! Hover text length: {len(hover_text)} chars"
                )
                print(
                    f"✅ Text is properly formatted as list with {len(goal_trace.text)} item(s)"
                )
            ...
            df_results = pd.DataFrame(
                {"date_str": ["2024-01-01"], "age_at_scan": [44.0], "almi_kg_m2": [8.0]}
            )
            ...
            assert goal_trace is not None, (
                "Goal trace should be present even with minimal data"
            )
            assert isinstance(goal_trace.text, (list, tuple)), (
                "Goal trace text should be a list or tuple"
            )
...
class TestOptionalBFGoalIntegration:
    ...
    def setup_user_and_scan_data(
        self, at, bf_percentage=25.0, training_level="intermediate"
    ):
    ...
    def test_optional_bf_athletic_range_maintains_current(self, app):
        ...
        if load_button:
            ...
            if analysis_button:
                ...
                # Verify no exceptions occurred
                assert len(at.exception) == 0, (
                    "Analysis with optional BF% should not cause exceptions"
                )
        ...
        if load_button:
            ...
            if analysis_button:
                ...
                for markdown in at.markdown:
                    if any(
                        keyword in markdown.value.lower()
                        for keyword in [
                            "try to add",
                            "maintain",
                            "lean mass",
                ...
                assert len(at.markdown) > 0, (
                    "Goal information should be displayed in markdown format"
                )
    ...
    def test_optional_bf_with_empty_goal_fields(self, app):
        ...
        if load_button:
            ...
            for num_input in at.number_input:
                if (
                    "body fat" in num_input.label.lower()
                    or "bf" in num_input.label.lower()
                ):
            ...
            if analysis_button:
                ...
                assert len(at.exception) == 0, (
                    "Empty BF% goals should be handled gracefully"
                )

                # Analysis should still complete and show results
                metrics_displayed = len(at.metric) > 0
                assert metrics_displayed, (
                    "Metrics should still be displayed with optional BF%"
                )
    ...
    def test_webapp_bf_goal_forms_present(self, app):
        ...
        for num_input in at.number_input:
            if (
                "body fat" in num_input.label.lower()
                and "optional" in num_input.label.lower()
            ):
        ...
        assert bf_goal_inputs_found >= 0, (
            "Optional BF% goal inputs should be present in goals section"
        )
        ...
        for num_input in at.number_input:
            if (
                "body fat" in num_input.label.lower()
                and num_input.help
                and "intelligent" in num_input.help.lower()
            ):
                break
    ...
    def test_optional_bf_different_health_categories(self, app):
        ...
        if load_button:
            ...
            if analysis_button:
                ...
                assert len(at.exception) == 0, (
                    "Different BF% health categories should be handled correctly"
                )
                ...
                any(
                    any(
                        ...
                        for keyword in ["maintain", "add", "target", "body fat"]
                    )
                ...
                assert len(markdown_content) > 0, (
                    "Goal-related content should be displayed"
                )
        ...
        if load_button:
            ...
            if training_selectbox:
                ...
                if analysis_button:
                    ...
                    assert len(at.exception) == 0, (
                        "Advanced training level should work with optional BF%"
                    )
    ...
    def test_optional_bf_backward_compatibility(self, app):
        ...
        if load_button:
            ...
            for _i, bf_input in enumerate(
                bf_inputs[:2]
            ):  # Limit to first 2 (ALMI, FFMI)
            ...
            if analysis_button:
                ...
                assert len(at.exception) == 0, (
                    "Explicit BF% goals should still work (backward compatibility)"
                )
