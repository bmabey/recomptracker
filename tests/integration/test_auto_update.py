...
def test_auto_update_logic():
    ...
    get_compact_config(session_state)
    has_data = (
        session_state.user_info.get("birth_date")
        or session_state.user_info.get("height_in", 0) != 66.0
        or len(session_state.scan_history) > 0
    )
