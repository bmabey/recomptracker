...
def test_url_sharing():
    ...
    user_info, scan_history, almi_goal, ffmi_goal = expand_compact_config(
        recovered_config
    )
    ...
    assert (
        scan_history[0]["total_weight_lbs"]
        == session_state.scan_history[0]["total_weight_lbs"]
    )

    assert (
        almi_goal["target_percentile"] == session_state.almi_goal["target_percentile"]
    )
    assert (
        ffmi_goal["target_percentile"] == session_state.ffmi_goal["target_percentile"]
    )
