...
from webapp import expand_compact_config, decode_state_from_url, initialize_session_state

# Import schema validation utilities
import sys
sys.path.append(str(Path(__file__).parent.parent.parent / "docs"))
from v1_schema_validator import validate_v1_config, validate_v1_url, load_v1_schema
...
class TestV1URLStateSpecification:
    ...
    def create_v1_edge_case_config(self):
        ...
        return {
            ...
            "s": [
                ["01/01/2024", 135.8, 95.2, 35.6, 26.2, 10.8, 32.4]
            ],
            "ag": {
    ...
    def test_v1_minimal_config_round_trip(self):
        ...
        url_valid, url_error, decoded_config = validate_v1_url(share_url, raise_on_error=False)
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = expand_compact_config(v1_config)
    ...
    def test_v1_complete_config_round_trip(self):
        ...
        # Validate URL too
        url_valid, url_error, decoded_from_url = validate_v1_url(share_url, raise_on_error=False)
    ...
    def test_v1_url_length_constraints(self):
        ...
        original_json = json.dumps({
            "user_info": {"birth_date": "06/15/1985", "height_in": 68.0, "gender": "male", "training_level": "novice"},
            "scan_history": [{"date": f"{(i%12)+1:02d}/{(i%28)+1:02d}/{2020+(i//12)}", 
                            "total_weight_lbs": 150.0+i*0.5, "total_lean_mass_lbs": 120.0+i*0.3,
                            "fat_mass_lbs": 25.0+i*0.1, "body_fat_percentage": 16.5+i*0.05,
                            "arms_lean_lbs": 15.0+i*0.1, "legs_lean_lbs": 38.0+i*0.2} for i in range(20)],
            "goals": {"almi": {"target_percentile": 0.85}, "ffmi": {"target_percentile": 0.80}}
        }, separators=(',', ':'))
        ...
        assert compression_ratio < 0.7, f"Compression ratio too low: {compression_ratio:.2f}"
...
class TestV1URLWebAppIntegration:
    ...
    def test_v1_url_session_state_initialization(self, mock_query_params):
        ...
        with patch('streamlit.session_state', mock_session_state), \
            ...
            for attr in required_attributes:
                assert hasattr(mock_session_state, attr), f"Missing session state attribute: {attr}"
            
            # Verify loaded data matches V1 config
            assert mock_session_state.user_info['birth_date'] == "08/15/1990"
            assert mock_session_state.analysis_results is None  # Should be initialized to None
            assert mock_session_state.url_loaded == True
    ...
    def test_v1_config_validation_rules(self):
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = expand_compact_config(invalid_config)
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = expand_compact_config(boundary_config)
...
class TestV1JSONSchemaValidation:
    ...
    def test_all_example_configs_validate(self):
        ...
        for config_name, config in configs:
            ...
            assert is_valid, f"{config_name} config should validate against schema: {error}"
...
class TestV1BackwardCompatibilityGuarantee:
    ...
    def test_v1_format_stability(self):
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = expand_compact_config(reference_config)
        
        # User info mapping
        assert user_info['birth_date'] == reference_config['u']['bd']
        assert user_info['height_in'] == reference_config['u']['h']
        assert user_info['gender'] == ('male' if reference_config['u']['g'] == 'm' else 'female')
    ...
    def test_v1_missing_optional_fields_handling(self):
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = expand_compact_config(minimal_config)
    ...
    def test_v1_future_compatibility_resilience(self):
        ...
        required_scan_fields = ['date', 'total_weight_lbs', 'total_lean_mass_lbs', 
