...
class TestURLStateLoadingIntegration:
    ...
    def test_decode_state_from_url_success(
        self, mock_query_params, example_config, mock_session_state
    ):
        ...
        mock_query_params.__getitem__ = MagicMock(
            return_value=urllib.parse.unquote(encoded_data)
        )
...
class TestWebAppURLIntegration:
    ...
    def test_webapp_loads_url_state_correctly(self, app, example_config_url):
        ...
        assert len(at.exception) == 0, (
            f"App should run without exceptions, got: {at.exception}"
        )
        ...
        assert any("Load Example" in btn for btn in buttons), (
            "Should have Load Example button"
        )
        assert any("Run Analysis" in btn for btn in buttons), (
    ...
    def test_webapp_url_loading_with_empty_params(self, app):
        ...
        url_load_messages = [
            msg for msg in success_messages if "Configuration loaded from URL" in msg
        ]
        assert len(url_load_messages) == 0, (
            "Should not show URL loading message in default state"
        )
...
class TestURLRoundTripIntegrity:
    ...
    def test_full_round_trip_integrity(self, example_config):
        ...
        assert compact_original == compact_recovered, (
            "Round-trip should preserve exact data"
        )

        # Expand recovered config
        user_info, scan_history, almi_goal, ffmi_goal, height_display = (
            expand_compact_config(compact_recovered)
        )
        ...
        assert (
            almi_goal["target_percentile"]
            == example_config["goals"]["almi"]["target_percentile"]
        )
        assert (
            ffmi_goal["target_percentile"]
            == example_config["goals"]["ffmi"]["target_percentile"]
        )
...
class TestURLEdgeCases:
    ...
    def test_malformed_base64_handling(self, mock_query_params):
        ...
        result = decode_state_from_url()
        assert not result, "Should return False for malformed data"
    ...
    def test_invalid_json_handling(self, mock_query_params):
        ...
        result = decode_state_from_url()
        assert not result, "Should return False for invalid JSON"
    ...
    def test_partial_config_handling(self):
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = (
            expand_compact_config(partial_compact)
        )
    ...
    def test_config_with_special_characters(self):
        ...
        special_compact = {
            ...
            "s": [["12/31/2023", 150.5, 120.3, 25.2, 16.8, 15.1, 38.9]],
            "ag": {"tp": 0.95, "ta": 30},
            "fg": {"tp": 0.80, "ta": 30},
        }
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = (
            expand_compact_config(special_compact)
        )
    ...
    def test_maximum_scan_limit_handling(self):
        ...
        user_info, scan_history, almi_goal, ffmi_goal, height_display = (
            expand_compact_config(max_compact)
        )
