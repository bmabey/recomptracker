...
def test_edge_cases():
    ...
    max_scans_config = {
        ...
        "s": [[f"01/{i+1:02d}/2024", 150.0 + i, 120.0, 25.0, 16.5, 15.0, 38.0] for i in range(20)],
        "ag": {"tp": 0.85},
        "fg": {"tp": 0.75}
    }
...
def test_automatic_analysis_conditions():
    ...
    should_auto_analyze = (
        user_info.get('birth_date') and 
        user_info.get('gender') and 
        len(scan_history) > 0 and
        any(scan.get('date') for scan in scan_history)
    )
    
    assert should_auto_analyze == True
    ...
    assert birth_date_check == False
    print("✅ Auto-analysis conditions (empty birth date logic): PASSED")
    
    # Test case 3: Empty scan dates should NOT trigger analysis
    has_valid_dates = any(scan.get('date', '').strip() for scan in [{'date': '', 'weight': 150}])
    assert has_valid_dates == False
