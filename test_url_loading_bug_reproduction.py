...
class MockSessionState:
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(f'st.session_state has no attribute "{name}". Did you forget to initialize it?')
...
def test_url_loading_session_state_bug():
    ...
    with patch('streamlit.query_params') as mock_query_params, \
        ...
        assert mock_session_state.has_attribute('user_info'), "user_info should be loaded from URL"
        assert mock_session_state.has_attribute('scan_history'), "scan_history should be loaded from URL"
        ...
        print("🔍 Attempting to access analysis_results (this should fail with the bug)...")
